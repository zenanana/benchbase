...
public class TaobenchReadZWriteXRecord extends Procedure {
     ...
     */
    public void run(Connection conn, Integer[] trx_args, int Z_start, int Z_end, String[] fields, String[] results) throws SQLException {
        ...
        for (int i = 0; i < YCSBConstants.FILLER_STMT_SIZE; i++) {
            ...
            //     }
            // } else {
                // System.out.printf("Write %d%n", randInt);
                try (PreparedStatement stmt = this.getPreparedStatement(conn, updateZStmt)) {
